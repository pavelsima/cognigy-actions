...
const InputPluginRenderer = ({
	...
	if (rulePlugin) {
		const emitAnalytics = (event: string, payload?: any) =>
			onEmitAnalytics(`plugin/${rulePlugin.name || "unknown"}/${event}`, payload);
	...
	const selectInputs = plugins.filter(plugin => plugin.type === "select") as SelectInputPlugin[];

	const matchedSelectInput = selectInputs.find(plugin => plugin.id === inputMode);
	...
	const emitAnalytics = (event: string, payload?: any) =>
		onEmitAnalytics(
			`plugin/${(matchedSelectInput && matchedSelectInput.name) || "unknown"}/${event}`,
			payload,
		);
	...
	return (
		<InputRoot
			className={classnames(
				"webchat-input",
				sttActive && "webchat-input-stt-active",
				textActive && "webchat-input-text-active",
			)}
		>
			{tabs}
			{matchedSelectInput && (
